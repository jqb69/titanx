...
def _new_token(username: str) -> str:
    ...
    to_encode = {"sub": username, "exp": int(expire.timestamp())}  # Unix timestamp
...
def login_user(username: str, password: str, current_ip: str, totp_code: str = None, phone_otp: str = None):
    username = (username or "").strip()
    user_key = f"user:{username.lower()}"
    ...
    known_ips = [ip for ip in stored.get("known_ips", "").split(",") if ip]
    ...
    if is_abnormal_ip:
        ...
        if totp_code:
            ...
            if not totp_secret:
                return None, "Google Authenticator not set up"
            ...
            if not phone:
                return None, "No phone number set"
            if not verify_otp(phone, phone_otp):
                return None, "Invalid phone OTP"
...
def get_totp_uri(username: str):
    stored = r.hgetall(f"user:{(username or '').strip().lower()}")
    if stored and stored.get("totp_secret"):
        return pyotp.TOTP(stored["totp_secret"]).provisioning_uri(name=username, issuer_name="MIKIE")
    return None
...
def google_login(google_token: str) -> Tuple[Optional[str], str]:
    try:
        ...
        return None, f"Google login failed: {e}"# web/auth.py
import os
import re
import secrets
from datetime import datetime, timedelta
